...
sudo -u postgres pg_dump -p 5433 -Fc appdb > /pgbackups/full/appdb_full_$(date +%F).dump
🔹 Schema-Only Backup
sudo -u postgres pg_dump -p 5433 -s appdb > /pgbackups/schema/appdb_schema_$(date +%F).sql
🔹 Data-Only Backup
sudo -u postgres pg_dump -p 5433 -a appdb > /pgbackups/data/appdb_data_$(date +%F).sql
#Backup Maintenance (Rotation Policy)
...
Outcome
*Implemented an automated PostgreSQL backup system
*Verified data recoverability through restore testing
*Simulated a disaster recovery scenario
